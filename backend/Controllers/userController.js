...
export const register = catchAsyncErrors(async (req, res, next) => {
  ...
  // To fill the required fields like name, email, phone, address, password, role
  if (!name || !email || !phone || !address || !password || !role) {
    return next(new ErrorHandler("All fields are required", 400));
  }

  // For job seekers looking for a specific role
...
export const updateProfile = catchAsyncErrors(async (req, res, next) => {
  const newUserData = {
    name: req.body.name,  // Corrected here
  ...
  if (req.files) {
    const resume = req.files.resume;  // Assuming req.files contains the resume
    if (resume) {
...
export const updatePassword=catchAsyncErrors(async(req,res,next)=>{
  ...
  {
    return next(new ErrorHandler("new Password & confirm new Password does not match",400));
  }
